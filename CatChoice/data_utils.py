...
def prepare_train_features(examples, args, tokenizer):
    ...
    for utter, service, slot, poss_values in zip(examples[args.utter_col], \
                                                examples[args.service_col], \
                                                examples[args.slot_col], \
                                                examples[args.poss_values_col]):
        if len(poss_values) < args.neg_num + 1:
            poss_values += ['' for i in range(args.neg_num + 1 - len(poss_values))]
        sentence_counts.append(len(paras))
        for v in poss_values:
            if pad_on_right:
                first_sentences.append(ques)
                second_sentences.append(utter)
            else:
                first_sentences.append(utter)
