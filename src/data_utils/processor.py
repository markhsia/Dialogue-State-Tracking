...
class SGDDataProcessor(object):
    ...
    def __init__(
        ...
        Args:
            task_name: task name, e.g. "train_dev"
        ...
        self._available_splits = list(DATASET_NAMES[task_name].keys())
        self.schemas = schemas
        self.schema_config = schema_config

        self._dir_names = {split: DATASET_NAMES[task_name][split] for split in self._available_splits}
        self._seen_services = {split: set() for split in self._available_splits}
        ...
        self.slots_relation_file = os.path.join(dialogues_example_dir, f"{task_name}_train_slots_relation_list.pkl")
        for dataset_split in self._available_splits:
            # Process dialogue files
            dial_file = f"{task_name}_{dataset_split}_examples.processed"
    ...
    def save_dialog_examples(self, overwrite_dial_files: bool): # Called by prepare_data() in sgd_model
        """
        Preprocesses dialogues and saves to disk.
        Args:
            overwrite_dial_files: whether or not to overwrite saved file if already exists
        """
        for dataset in self._available_splits:
            dial_file = self.dial_files[(self._task_name, dataset)]
            id_file = self.id_files[(self._task_name, dataset)]
            if not os.path.exists(dial_file) or overwrite_dial_files:
                ...
                dial_examples, dial_ids, slots_relation_list = self._generate_dialog_examples(
    ...
    def get_dialog_examples(self, dataset_split: str) -> List[object]:  # called by data_utils.dataset.SGDDataset
        """
        ...
        id_file = self.id_files[(self._task_name, dataset_split)]
        logging.info(f"Loading dialogue ids from {id_file}.")
        with open(id_file, "rb") as f:
            dial_ids = pickle.load(f)
    ...
    # Called by save_dialog_examples
        ...
        dialogs = self.dialogs[(self._task_name, dataset_split)]
        ...
        for dialog_idx, dialog in enumerate(dialogs):
            ...
            examples.extend(
                self._create_examples_from_dialog(dialog_idx, dialog, schemas, 
                                                dataset_split, slot_carryover_candlist, subsample)
            )
            dialog_ids.append(dialog["dialogue_id"])
        ...
        return examples, dialog_ids, slots_relation_list

    def _create_examples_from_dialog(
        self, dialog_idx: int, dialog: dict, schemas: object, 
        dataset_split: str, slot_carryover_candlist: dict, subsample: bool
    ):  # Called by _generate_dialog_examples
        """
        ...
        for turn_idx, turn in enumerate(dialog["turns"]):
            ...
            if turn["speaker"] == "USER":
                ...
                if turn_idx > 0:
                    system_turn = dialog["turns"][turn_idx - 1] # preceeding system turn
                ...
                turn_id = "{}-{:03d}-{:02d}".format(dataset_split, dialog_idx, turn_idx)
    ...
    def _create_examples_from_turn(
        ...
        schemas: object,
        subsample: bool,
    ):  # Called by _create_examples_from_dialog
        """
        ...
        for service, user_frame in user_frames.items():
            ...
            dataset_split, dialog_idx, turn_id_ = turn_id.split('-')
    ...
    @classmethod
    def get_dialogue_files(cls, data_dir: str, task_name: str, dataset_split: str):
        """
        ...
        """
        files = []
        for d in DATASET_NAMES[task_name][dataset_split]:
            files += sorted(glob.glob(os.path.join(os.path.join(data_dir, d), "dialogues_*.json")))
